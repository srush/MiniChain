...
from typing import List, Set, Callable, Any, Tuple, Union, Dict
...
@dataclass
class Constructor:
    fns: List[Callable[[Dict[Block, Any]], Dict[Block, Any]]] = field(default_factory=list)
    ...
    def merge(self, other: "Constructor") -> "Constructor":
        return Constructor(self.fns + other.fns,
...
def api_keys(keys: Set[str]={"OPENAI_API_KEY"}) -> None:
    ...
    with gr.Accordion(label="API Keys", elem_id="json", open=False):        
        if "OPENAI_API_KEY" in keys and "OPENAI_API_KEY" not in os.environ:
            key_names["OPENAI_API_KEY"] = gr.Textbox(os.environ.get("OPENAI_API_KEY"), label="OpenAI Key", elem_id="json", type="password")
            gr.Markdown("""
    ...
    def api_run(data): # type: ignore
        for k, v in key_names.items():
            if data[v] is not None and data[v] != "":
                os.environ[k] = data[v]
        return {}
    api_btn.click(api_run,
                  inputs=set(key_names.values()))


def show(prompt: Prompt[Any, Any, Any], 
